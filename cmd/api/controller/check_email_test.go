...
func MockGet(c *gin.Context, email string) {
	...
	c.Request.URL = &url.URL{
		...
		User:        nil,
		Host:        "localhost",
		Path:        "/api/v1/superhero_screen/check_email",
		RawPath:     "",
		ForceQuery:  false,
		RawQuery:    "email=" + email,
		Fragment:    "",
		RawFragment: "",
	}
}

func TestController_CheckEmail(t *testing.T) {
	...
	mockService := service.New(mockEs)
	...
	MockGet(ctx, "test@test.com")

	mockController.CheckEmail(ctx)
